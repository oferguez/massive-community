...
- `og/` is the active research workspace, with fetchers, models, and utilities.
- `og/data/` stores local DuckDB files and schema docs (see `og/data/schemas_desc.md`).
- `images/` holds assets; `logs/` and `obs/` store experiment outputs.
- `og/models/` contains the generic models used for trading logic.


## Build, Test, and Development Commands
- Most examples are standalone; follow each `README.md` under `examples/` for setup.
- For the `og` workspace, run scripts directly, e.g. `python og/main.py`.
- DuckDB exploration happens via `og/data/` and helper scripts in `og/data`.
...
- Linting is optional; a minimal `pylint` config exists in `og/pyproject.toml`. Keep suppressions minimal and revisit once `og` is extracted.
...
- For `og`, keep tests in `og/tests` and run `python -m unittest` from that folder.
